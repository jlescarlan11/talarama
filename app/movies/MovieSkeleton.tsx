  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3 sm:gap-4 md:gap-5">
      {Array.from({ length: 10 }).map((_, index) => (
        <div key={index} className="group aspect-[2/3] rounded-lg overflow-hidden relative cursor-pointer shadow-md">
          {/* Poster Skeleton */}
          ...
          <div className="absolute bottom-0 left-0 right-0 p-4 text-white transform translate-y-full group-hover:translate-y-0 transition-transform duration-300 ease-out">
            <div className="text-center space-y-2">
              ...
              <div className="flex flex-wrap gap-1 justify-center mt-2">
                ...
                <div className="h-5 bg-gray-300 rounded-full w-14 animate-pulse"></div>
              </div>
            </div>
          </div>
